...
// TestSchema_NoAccessPolicy pins FR-010a: nc2_azure_cluster MUST
// NOT expose `access_policy` (AWS-only).
func TestSchema_NoAccessPolicy(t *testing.T) {
	t.Parallel()

	s := resourceSchema(context.Background())
	if _, ok := s.Attributes["access_policy"]; ok {
		t.Errorf("nc2_azure_cluster schema includes access_policy; FR-010a says it must be AWS-only")
	}
}

// TestSchema_NoDesiredState pins FR-012: nc2_azure_cluster MUST
// NOT expose `desired_state` — hibernate / resume is AWS-only.
func TestSchema_NoDesiredState(t *testing.T) {
	t.Parallel()

	s := resourceSchema(context.Background())
	if _, ok := s.Attributes["desired_state"]; ok {
		t.Errorf("nc2_azure_cluster schema includes desired_state; FR-012 says hibernate/resume must be AWS-only")
	}
}
