...
| Phase 2 — Foundational (T011–T045) | redact, oapi, auth, audit, client, provider, orgshared, dsshared, clustershared, main.go, CI tools | **Complete** including T025 untrusted-cert TLS test (`internal/client/tls_test.go`), T034 import-graph guard (`internal/provider/imports_test.go`), T039–T042 `tools/coverage-check` + `tools/sensitive-lint` with synthetic-fixture tests, T044 provider-server smoke test (`main_test.go`). Tools use stdlib JSON walking instead of kin-openapi to tolerate the upstream spec's non-canonical `items: "<RefName>"` strings. |
| Phase 3 — US1 MVP (T046–T090) | nc2_organization, nc2_cloud_account, nc2_cloud_account_region + 6 data sources | **Complete**: all three managed resources implemented with schema/sensitive/CRUD/import/openapi-mapping/README; the 6 data sources (`nc2_organizations`, `nc2_organization`, `nc2_organization_audit_trail`, `nc2_cloud_accounts`, `nc2_cloud_account`, `nc2_cloud_account_regions`) implemented with schema/Read/openapi-mapping/README; provider wired in `internal/provider/registry_us1.go`; 9 examples under `examples/{resources,data-sources}/...`; 9 doc pages under `docs/{resources,data-sources}/...`; coverage-check shows 49/49 covered. Full unit-test suite green. |
| Phase 4 — US2 (T091–T126) | nc2_aws_cluster, nc2_azure_cluster, nc2_gcp_cluster + cluster data sources | **Implementations + depth tests**: lifecycle and update routing covered by `routing_test.go` (full FR-010 ordered diff), schema_test.go (FR-010a AWS-only `access_policy` guard for all 3 resources). |
| Phase 5 — US3 (T127–T151) | 10 inventory / discovery data sources | **Implementations + smoke + operation-mapping coverage tests**. Acceptance tests under `tests/acceptance/` still gated by `TF_ACC=1` and require live NC2 sandbox. |
| Phase 6 — US4 (T152–T164) | desired_state hibernate/resume on the 3 cluster resources | **Complete**: per-cluster `hibernate_test.go` for AWS / Azure / GCP covering hibernate-only routing, resume-only routing, combined-diff rejection, and idempotency. |
| Phase 7 — US5 (T165–T188) | 8 actions + shared scaffolding | **Implementations + smoke + operation-mapping coverage tests**. Per-action acceptance tests gated by `TF_ACC=1`. |
| Phase 8 — Polish (T189–T212) | Release pipeline, supply-chain attestation, docs guides, integration tests | **Critical paths complete**: `tests/integration/no_skip_verify_test.go` (FR-003c, T191), `no_external_secret_stores_test.go` (FR-001b, T192), `coverage_test.go` (T190), `docs_coverage_test.go` (FR-029, T193), `godoc_coverage_test.go` (SC-007, T194 via stdlib AST instead of `revive` shell-out). `docs/index.md`, four guides (getting-started, authentication, security-hardening, async-and-tasks), `CHANGELOG.md` shipped. `.goreleaser.yml` + `.github/workflows/release.yml` + `terraform-registry-manifest.json` ready for first tag. Per-resource sensitive aggregation test (T189), `act`-based release workflow simulation (T202), and live SC-004 timing (T211/T212) remain as out-of-band tasks. |

`go test ./...` is green across every package, race detector clean for `auth`/`oapi`/`client`. `go vet ./...` clean. `tools/coverage-check` reports 49/49 OpenAPI operations covered.
